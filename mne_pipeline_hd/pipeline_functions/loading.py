...
class Group(BaseLoading):
    ...
    def init_attributes(self):
        ...
        # The assigned event-id
        if self.group_list[0] not in self.pr.meeg_event_id:
            self.pr.meeg_event_id[self.group_list[0]] = dict()
            if not self.suppress_warnings:
                print(f'No EventID assigned for {self.name}, defaulting to empty dictionary')
        self.event_id = self.pr.meeg_event_id[self.group_list[0]]

        # The selected trials from the event-id
        if self.group_list[0] not in self.pr.sel_event_id:
            self.pr.sel_event_id[self.group_list[0]] = list()
            if not self.suppress_warnings:
                print(f'No Trials selected for {self.name}, defaulting to empty list')
        self.sel_trials = self.pr.sel_event_id[self.group_list[0]]
